...
func UpdateAssetsSubpathInDir(subpath, directory string) error {
	...
	reCSP := regexp.MustCompile(`<meta http-equiv="Content-Security-Policy" content="script-src 'self' cdn.rudderlabs.com/ twemoji.maxcdn.com/ js.stripe.com/v3([^"]*)">`)
	...
	newRootHTML = reCSP.ReplaceAllLiteralString(newRootHTML, fmt.Sprintf(
		`<meta http-equiv="Content-Security-Policy" content="script-src 'self' cdn.rudderlabs.com/ twemoji.maxcdn.com/ js.stripe.com/v3%s">`,
